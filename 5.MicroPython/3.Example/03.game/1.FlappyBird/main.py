...
class Obstacle:
    def __init__(self, x, size):
        self.size = size
        self.gap = randint(6 + self.size, HEIGHT - 6 - self.size)     # 随机生成间隙大小
        self.x = x              # 距离鸟大小
        self.score = 0          # 分数
        self.rate = 3           # 速率
    ...
    def scroll(self):
        ...
        if self.x < -pipe_size[0]:
            self.score += 1
            self.x = WIDTH
            self.gap = randint(6 + self.size, HEIGHT - 6 - self.size)
...
class Game():
   ...
   # 绘制
    ...
    def _clicked(self):
        if button.value() == 0 and not self.pressed:
            self.pressed = True
            return True
        elif button.value() == 1 and self.pressed:
            self.pressed = False
        return False
